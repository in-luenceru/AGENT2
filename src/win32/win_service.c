...
static LPTSTR g_lpszServiceName        = "MonitoringAgentSvc";
static LPTSTR g_lpszServiceDisplayName = "Monitoring Agent";
static LPTSTR g_lpszServiceDescription = "Monitoring Agent Windows Agent";
...
int os_stop_service()
{
    ...
    * newer instances of monitoring-agent.
    */
    ...
    if (ossecServiceStatusHandle) {
        switch (dwOpcode) {
            case SERVICE_CONTROL_STOP:
                ...
                // Kill children processes spawned by modules, only in monitoring-agent
                wm_kill_children();
                stop_wmodules();
